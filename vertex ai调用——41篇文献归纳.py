...
try:
    ...
    prompt_extraction = """
        ...
        【表头结构与填写要求（必须严格遵守以下 24 列绝对顺序，绝不可擅自合并或增删列）】
        1. 文献序号：(文件名中带有的：文献x，填写数字x即可)
        2. 文献名称：(文献标题全称)
        3. 文章类型：(研究，综述，会议 )
        4. 第一作者：(提取第一作者姓名)
        ...
        8. DOI：(提取 DOI 编号)
        9. 电池类型判定：(填写：首先判断是否是综述，不是综述则根据电池组装使用的是液态电解质还是固态电解质还是没有装电池填写液态还是固态还是无电池，液态 / 固态 / 综述/ 无电池)
        10. 包覆材料化学式
        11. 包覆材料重量占比
        ...
        18. 电池装配配置-负极物质化学式
        19. 电池装配配置-电解质化学式
        20. 电化学性能-首圈-比容量mAh/g
        21. 电化学性能-首圈-充放电效率%
        22. 电化学性能-倍率：(格式：0.5C=xxxmAh/g；1C=xxxmAh/g；...)
        23. 电化学性能-长循环：(格式：(测试倍率, 圈数, 保持率%))
        24. 核心机制深度解析：(禁止简略泛泛而谈，必须深入具体数据与现象！格式：[测试手段] -> [具体实验现象与数据] -> [推导出的微观机理]。示例：【XRD】充放电过程中(003)峰在高压下往低角度的偏移幅度显著减小 -> 证明包覆层有效抑制了H2到H3的不利相变及c轴塌陷；【XPS/XAS】O 1s谱中晶格氧结合能发生偏移，且观测到P-O-M键的形成 -> 证实包覆层与基体发生界面化学杂化，锚定了表面晶格氧，抑制了高压下的剧烈析氧副反应。)
        """
